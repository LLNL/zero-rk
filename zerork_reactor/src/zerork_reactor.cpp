...
void zerork_reactor(int inp_argc, char **inp_argv)
{
  ...
  zerork_reactorIFP inputFileDB(inp_argv[1]);
  ...
  int cvcp = inputFileDB.constantPressure(); //WARNING: Converting bool to int
  const char* cklogfile = inputFileDB.mechLogFile().c_str();
  const char* reactorlogfile = inputFileDB.outFile().c_str();
  int multireac = inputFileDB.cuda();
  int gpu_id = inputFileDB.cudaDevice();
  int n_reactors_max = inputFileDB.nReactorsMax();
  int n_reactors_min = inputFileDB.nReactorsMin();

  double logiA = inputFileDB.logiA();
  double logiK = inputFileDB.logiK();
  double logiQ = inputFileDB.logiQ();
  double logiM = inputFileDB.logiM();
  double logiB = inputFileDB.logiB();
  double logirNu = inputFileDB.logirNu();
  double min_scaled_dt = inputFileDB.min_scaled_dt();
  ...
  if( inputFileDB.linearSolver() == std::string("DirectDense") ) {
    ...
    lsolver = 1;
  } else if( inputFileDB.linearSolver() == std::string("DirectSparse") ) {
    lsolver = 2;
  } else if( inputFileDB.linearSolver() == std::string("DirectSparseDVD") ) {
    lsolver = 3;
  } else if( inputFileDB.linearSolver() == std::string("IterativeSparse") ) {
    lsolver = 4;
  } else if( inputFileDB.linearSolver() == std::string("IterativeSparseCusolverRf") ) {
    lsolver = 5;
  } else if( inputFileDB.linearSolver() == std::string("IterativeSparseCusolverRfBlocked") ) {
    lsolver = 6;
  } else if( inputFileDB.linearSolver() == std::string("IterativeSparseCusolverRfBatched") ) {
    lsolver = 7;
  } else if( inputFileDB.linearSolver() == std::string("IterativeSparseCusolverSpBatched") ) {
    lsolver = 8;
  } else {
    printf("Huh? %s\n",inputFileDB.linearSolver().c_str());
    exit(-1);
  }

  int cusolver_factor_alg = inputFileDB.CUSOLVER_FACTOR_ALG();
  int cusolver_solve_alg = inputFileDB.CUSOLVER_SOLVE_ALG();

  int verbosity = 5;

  //Setup reactor_lib
  zerork_reactor_setup_full(verbosity,cv_maxsteps,cv_maxord,cv_rtol,cv_atol,
                         abstol_dens,
                         cv_maxdt_internal,cv_sparsethresh,cv_nlconvcoef,
                         cv_epslin,lsolver,cvcp,
                         inputFileDB.mechFile().c_str(),
                         inputFileDB.thermFile().c_str(),
                         cklogfile,reactorlogfile,
                         n_reactors_max,n_reactors_min,
  ...
  zerork_solve_reactors(nReactors, tend, &reactorT[0], &reactorP[0],
                     &reactorDPDT[0], &reactorMassFrac[0],
                     &reactorCost[0], &reactorGpu[0]);
  zerork_print_stats();
