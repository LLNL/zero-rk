...
#include "const_pressure_reactor.h"


// implementation class for the ConstPressureReactor
class ConstPressureReactor::Impl: public ReactorBase
{
 public:
  ...
  ReactorError GetTimeDerivativeDiffusionSteady(const double state[],
                                                const double step_limiter[],
                                                double derivative[]);

  ReactorError GetTimeDerivativeDiffusionSteadyFixT(const double state[],
                                                    const double step_limiter[],
                                                    double derivative[]);
  ...
  ReactorError GetJacobianDiffusionSteady(const double state[],
                                          const double step_limiter[],
                                          double jacobian[]);

  ReactorError GetJacobianDiffusionSteadyFixT(const double state[],
                                              const double step_limiter[],
                                              double jacobian[]);
  ...
  ReactorError GetJacobianLimiterDiag(const double reactor_time,
				      const double state[],
				      const double step_limiter[],
				      double jacobian[]);
 ...
 private:
  ...
  ReactorError GetSparseJacobianSteady(const double state[],
				       ...
				       const bool Tfix,
                                       const double step_limiter[],
				       double jacobian[]);
  ReactorError GetSparseJacobianDiffusionSteady(const double state[],
                                                const double step_limiter[],
                                                double jacobian[]);

  ReactorError GetSparseJacobianDiffusionSteadyFixT(const double state[],
                                                    const double step_limiter[],
                                                    double jacobian[]);
  ReactorError GetSparseJacobianLimiter(const double reactor_time,
					...
					const double step_limiter[],
					double jacobian[]);
  ReactorError GetDiagJacobianLimiter(const double reactor_time,
					const double state[],
					const double step_limiter[],
					double jacobian[]);
  ...
  void GetJacobianColumnFromPerturbationSteady(const int column_id,
					       ...
					       double perturbed_derivative[],
					       double jacobian_column[]);
  void GetJacobianColumnFromPerturbationDiffusionSteady(const int column_id,
                                                        const double dstate,
                                                        const double original_state[],
                                                        const double original_derivative[],
                                                        const double step_limiter[],
                                                        ...
                                                        double perturbed_derivative[],
                                                        double jacobian_column[]);
  void GetJacobianColumnFromPerturbationDiffusionSteadyFixT(const int column_id,
                                                            const double dstate,
                                                            const double original_state[],
                                                            const double original_derivative[],
                                                            const double step_limiter[],
...
ConstPressureReactor::Impl::Impl(const char mechanism_name[],
                                 const char thermodynamics_name[],
                                 const char parser_log_name[],
                                 const MatrixType matrix_type,
	                         const double pressure)
{
  ...
  state_names.push_back(std::string("RelativeVolume"));
  ...
  info  = "Constant pressure reactor\n";
...
ConstPressureReactor::Impl::~Impl()
{
  DestroyMechanism();
}

ReactorError
  ConstPressureReactor::Impl::GetTimeDerivative(const double reactor_time,
                                                const double state[],
                                                double derivative[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetTimeDerivativeSteady(const double state[],
                                                      const double step_limiter[],
						      double derivative[])
{
  // define local constants of class members to enable loop vectorization
  ...
  // for steady premixed flames, state[num_species] contains mass flux
...
ReactorError
  ConstPressureReactor::Impl::GetTimeDerivativeDiffusionSteady(const double state[],
                                                               const double step_limiter[],
                                                               double derivative[])
{
  // define local constants of class members to enable loop vectorization
  ...
  double mix_molecular_mass = 0.0;
  for(int j=0; j<num_species; ++j) {
     mix_molecular_mass += state[j]*inv_molecular_mass_[j];
  }
  mix_molecular_mass = 1.0/mix_molecular_mass;
  ...
  // compute the rate of change of the relative volume
  // residual = v - RT/PWmix instead of dv/dt = v/T * dT/dt + RuT/p * \sum_i (1/mw[i] * dy[i]/dt)
  // enforce ideal gas law
  derivative[num_species] = relative_volume - RuT/pressure/mix_molecular_mass;
...
ReactorError
  ConstPressureReactor::Impl::GetTimeDerivativeDiffusionSteadyFixT(const double state[],
                                                                   const double step_limiter[],
                                                                   double derivative[])
{
  // define local constants of class members to enable loop vectorization
...
ReactorError
  ConstPressureReactor::Impl::GetTimeDerivativeLimiter(const double reactor_time,
						       const double state[],
						       const double step_limiter[],
						       double derivative[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetJacobian(const double reactor_time,
                                          const double state[],
                                          double jacobian[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetJacobianSteady(const double state[],
						const double convective[],
						const bool Tfix,
                                                const double step_limiter[],
						double jacobian[])
{
  ...
  if(GetMatrixType() == DENSE_COL_MAJOR) {
    ...
    return GetSparseJacobianSteady(state, convective, Tfix, step_limiter, jacobian);
  }
...
ReactorError
  ConstPressureReactor::Impl::GetJacobianDiffusionSteady(const double state[],
                                                         const double step_limiter[],
                                                         double jacobian[])
{
  ...
  if(GetMatrixType() == DENSE_COL_MAJOR) {
    ...
    return GetSparseJacobianDiffusionSteady(state, step_limiter, jacobian);
  }
...
ReactorError
  ConstPressureReactor::Impl::GetJacobianDiffusionSteadyFixT(const double state[],
                                                             const double step_limiter[],
                                                             double jacobian[])
{
  ...
  if(GetMatrixType() == DENSE_COL_MAJOR) {
    ...
    return GetSparseJacobianDiffusionSteadyFixT(state, step_limiter, jacobian);
  }
...
ReactorError
  ConstPressureReactor::Impl::GetJacobianLimiter(const double reactor_time,
						 const double state[],
						 const double step_limiter[],
						 double jacobian[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetJacobianLimiterDiag(const double reactor_time,
						     const double state[],
						     const double step_limiter[],
						     double jacobian[])
{

  return GetDiagJacobianLimiter(reactor_time, state, step_limiter, jacobian);

  return UNKNOWN;
}

int ConstPressureReactor::Impl::GetJacobianPattern(int row_id[],
                                                   int column_id[])
{
...
int ConstPressureReactor::Impl::BuildSparseJacobianArrays()
{
  ...
  const int num_species = mechanism_ptr->getNumSpecies();
  const int num_steps   = mechanism_ptr->getNumSteps();
  const int num_states  = num_species + 2; // relative volume/mass flux & temperature
...
ReactorError
  ConstPressureReactor::Impl::GetSparseJacobian(const double reactor_time,
                                                const double state[],
			                        double jacobian[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetSparseJacobianSteady(const double state[],
						      const double convective[],
						      const bool Tfix,
                                                      const double step_limiter[],
						      double jacobian[])
...
ReactorError
  ConstPressureReactor::Impl::GetSparseJacobianDiffusionSteady(const double state[],
                                                               const double step_limiter[],
                                                               double jacobian[])
{
  const double perturb_factor = 1.0e-8;
  ...
  double mix_mass_cp, RuT, mix_molecular_mass;
  double mass_sum, enthalpy_sum;
  double d_temperature, d_relative_volume;
  double min_concentration;
  zerork::mechanism *mechanism_ptr;
  ...
  for(int j=0; j<num_species; ++j) {
    ...
    enthalpy_sum +=
      original_derivative_[j]*enthalpies_[j]*inv_molecular_mass_[j];
    // enthalpy sum is non-dimensionalized bu RuT
    mix_molecular_mass += original_state_[j]*inv_molecular_mass_[j];
  }
  mix_molecular_mass = 1.0/mix_molecular_mass;
  ...
  for(int j=0; j<num_species; ++j) {
    ...
    //jacobian[sparse_id] = (RuT/pressure)*jacobian_sum +
    //  relative_volume*(ref_temperature/temperature)*jacobian[sparse_id+1];
    jacobian[sparse_id] = 0.0; //Set to 0 for now
  }

  // perturb the relative volume
  d_relative_volume = original_state_[num_species]*perturb_factor;
  GetJacobianColumnFromPerturbationDiffusionSteady(num_species,
                                                   d_relative_volume,
                                                   &original_state_[0],
                                                   &original_derivative_[0],
                                                   &step_limiter[0],
  ...
  // perturb the temperature
  d_temperature = original_state_[num_species+1]*perturb_factor;
  GetJacobianColumnFromPerturbationDiffusionSteady(num_species+1,
					  d_temperature,
					  &original_state_[0],
					  &original_derivative_[0],
                                                   &step_limiter[0],
...
ReactorError
  ConstPressureReactor::Impl::GetSparseJacobianDiffusionSteadyFixT(const double state[],
                                                                   const double step_limiter[],
                                                                   double jacobian[])
{
  const double perturb_factor = 1.0e-8;
  ...
  // compute the rate of temperature change
  // dT/dt = 0.0;
  original_derivative_[num_species+1] = 0.0;
  ...
  for(int j=0; j<num_species; ++j) {
    ...
    //specific_heats_[j]*inv_molecular_mass_[j];
    //jacobian[sparse_id+1] /= mix_mass_cp;
    jacobian[sparse_id+1] = 0.0; //Set to 0 for now
  }
  ...
  // perturb the relative volume
  d_relative_volume = original_state_[num_species]*perturb_factor;
  GetJacobianColumnFromPerturbationDiffusionSteadyFixT(num_species,
                                                       d_relative_volume,
                                                       &original_state_[0],
                                                       &original_derivative_[0],
                                                       &step_limiter[0],
  ...
  // perturb the temperature
  d_temperature = original_state_[num_species+1]*perturb_factor;
  GetJacobianColumnFromPerturbationDiffusionSteadyFixT(num_species+1,
                                                       d_temperature,
                                                       &original_state_[0],
                                                       &original_derivative_[0],
                                                       &step_limiter[0],
...
ReactorError
  ConstPressureReactor::Impl::GetSparseJacobianLimiter(const double reactor_time,
						       const double state[],
						       const double step_limiter[],
						       double jacobian[])
{
...
ReactorError
  ConstPressureReactor::Impl::GetDiagJacobianLimiter(const double reactor_time,
						     const double state[],
						     const double step_limiter[],
						     double jacobian[])
{
...
void ConstPressureReactor::Impl::
...
void ConstPressureReactor::Impl::
  GetJacobianColumnFromPerturbationDiffusionSteady(const int column_id,
                                                   const double dstate,
                                                   const double original_state[],
                                                   const double original_derivative[],
                                                   const double step_limiter[],
                                                   ...
                                                   double jacobian_column[])
{
  ...
  // compute the time derivative at the perturbed state
  GetTimeDerivativeDiffusionSteady(perturbed_state,step_limiter,perturbed_derivative);
...
void ConstPressureReactor::Impl::
  GetJacobianColumnFromPerturbationDiffusionSteadyFixT(const int column_id,
                                                       const double dstate,
                                                       const double original_state[],
                                                       const double original_derivative[],
                                                       const double step_limiter[],
                                                       ...
                                                       double jacobian_column[])
{
  ...
  // compute the time derivative at the perturbed state
  GetTimeDerivativeDiffusionSteadyFixT(perturbed_state,step_limiter,perturbed_derivative);
...
int ConstPressureReactor::Impl::GetNetStoichiometry(const int species_id,
                                                    const int step_id)
{
  zerork::mechanism *mechanism_ptr;
  mechanism_ptr = GetMechanism();
...
void ConstPressureReactor::Impl::SetReferenceTemperature(const double ref_temperature)
{
...
double ConstPressureReactor::Impl::GetReferenceTemperature() const
{
  return ref_temperature_;
}
void ConstPressureReactor::Impl::SetPressure(const double pressure)
{
  pressure_ = pressure;
  if(pressure_ <= 0.0) {
...
double ConstPressureReactor::Impl::GetPressure() const
{
  return pressure_;
}

double
  ConstPressureReactor::Impl::GetMixtureSpecificHeat_Cp(const double state[])
{
  const int num_species            = GetNumSpecies();
  const double ref_temperature     = ref_temperature_;
  const double temperature         = ref_temperature*state[num_species+1];
...
double
ConstPressureReactor::Impl::GetMixtureSpecificHeat_Cp(const double temperature,
                                                 const double mass_fractions[],
                                                 double *species_cp)
{
...
// ---------------------------------------------------------------------------
// Public facing API
ConstPressureReactor::ConstPressureReactor(const char mechanism_name[],
                                           const char thermodynamics_name[],
                                           const char parser_log_name[],
                                           const MatrixType matrix_type,
                                           const double pressure)
...
ReactorError
  ConstPressureReactor::GetTimeDerivative(const double reactor_time,
                                           const double state[],
                                           double derivative[])
{
...
ReactorError
  ConstPressureReactor::GetTimeDerivativeSteady(const double state[],
                                                const double step_limiter[],
						double derivative[])
{
...
ReactorError
  ConstPressureReactor::GetTimeDerivativeDiffusionSteady(const double state[],
                                                         const double step_limiter[],
						double derivative[])
{
  return impl_->GetTimeDerivativeDiffusionSteady(state,step_limiter,&derivative[0]);
}

ReactorError
  ConstPressureReactor::GetTimeDerivativeDiffusionSteadyFixT(const double state[],
                                                             const double step_limiter[],
						double derivative[])
{
  return impl_->GetTimeDerivativeDiffusionSteadyFixT(state,step_limiter,&derivative[0]);
}

ReactorError
  ConstPressureReactor::GetTimeDerivativeLimiter(const double reactor_time,
						 const double state[],
						 const double step_limiter[],
						 double derivative[])
{
...
ReactorError ConstPressureReactor::GetJacobian(const double reactor_time,
					       const double state[],
					       double jacobian[])
{
...
ReactorError ConstPressureReactor::GetJacobianSteady(const double state[],
						     const double convective[],
						     const bool Tfix,
                                                     const double step_limiter[],
						     double jacobian[])
{
  return impl_->GetJacobianSteady(state,convective,Tfix,step_limiter,&jacobian[0]);
}

ReactorError ConstPressureReactor::GetJacobianDiffusionSteady(const double state[],
                                                              const double step_limiter[],
                                                              double jacobian[])
{
  return impl_->GetJacobianDiffusionSteady(state,step_limiter,&jacobian[0]);
}

ReactorError ConstPressureReactor::GetJacobianDiffusionSteadyFixT(const double state[],
                                                                  const double step_limiter[],
                                                              double jacobian[])
{
  return impl_->GetJacobianDiffusionSteadyFixT(state,step_limiter,&jacobian[0]);
}

ReactorError ConstPressureReactor::GetJacobianLimiter(const double reactor_time,
						      const double state[],
						      const double step_limiter[],
						      double jacobian[])
{
...
ReactorError ConstPressureReactor::GetJacobianLimiterDiag(const double reactor_time,
							  const double state[],
							  const double step_limiter[],
							  double jacobian[])
{
  return impl_->GetJacobianLimiterDiag(reactor_time,state,step_limiter,&jacobian[0]);
}

int ConstPressureReactor::GetJacobianPattern(int row_id[],
                                              int col_id[])
{
  return impl_->GetJacobianPattern(&row_id[0],&col_id[0]);
}

int ConstPressureReactor::GetNumStates() const
{
  return impl_->GetNumStates();
}

int ConstPressureReactor::GetNumSpecies() const
{
  return impl_->GetNumSpecies();
}

int ConstPressureReactor::GetNumReactions() const
{
  return impl_->GetNumReactions();
}

int ConstPressureReactor::GetNumSteps() const
{
  return impl_->GetNumSteps();
}

int ConstPressureReactor::GetJacobianSize() const
{
  return impl_->GetJacobianSize();
}

MatrixType ConstPressureReactor::GetMatrixType() const
{
  return impl_->GetMatrixType();
}

const char * ConstPressureReactor::GetMechanismName() const
{
  return impl_->GetMechanismName();
}

const char * ConstPressureReactor::GetThermodynamicsName() const
{
  return impl_->GetThermodynamicsName();
}

const char * ConstPressureReactor::GetParserLogName() const
{
  return impl_->GetParserLogName();
}

const char * ConstPressureReactor::GetReactorInfo() const
{
  return impl_->GetReactorInfo();
}

zerork::mechanism* ConstPressureReactor::GetMechanism()
{
  return impl_->GetMechanism();
}

// species/reaction accessors
int ConstPressureReactor::GetIdOfState(const char *state_name) const
{
  return impl_->GetIdOfState(state_name);
}

const char * ConstPressureReactor::GetNameOfStateId(const int state_id) const
{
...
double ConstPressureReactor::GetAMultiplierOfForwardReactionId(const int reaction_id) const
{
  return impl_->GetAMultiplierOfForwardReactionId(reaction_id);
}

ReactorError ConstPressureReactor::SetAMultiplierOfForwardReactionId(const int reaction_id, const double a_multiplier)
{
  return impl_->SetAMultiplierOfForwardReactionId(reaction_id,a_multiplier);
}

double ConstPressureReactor::GetAMultiplierOfReverseReactionId(const int reaction_id) const
{
  return impl_->GetAMultiplierOfReverseReactionId(reaction_id);
}

ReactorError ConstPressureReactor::SetAMultiplierOfReverseReactionId(const int reaction_id, const double a_multiplier)
{
  return impl_->SetAMultiplierOfReverseReactionId(reaction_id, a_multiplier);
}

double ConstPressureReactor::GetAMultiplierOfStepId(const int step_id) const
{
  return impl_->GetAMultiplierOfStepId(step_id);
}

ReactorError ConstPressureReactor::SetAMultiplierOfStepId(const int step_id, const double a_multiplier)
{
  return impl_->SetAMultiplierOfStepId(step_id,a_multiplier);
}

void ConstPressureReactor::SetReferenceTemperature(const double ref_temperature)
{
  impl_->SetReferenceTemperature(ref_temperature);
}
double ConstPressureReactor::GetReferenceTemperature() const
{
  return impl_->GetReferenceTemperature();
}

void ConstPressureReactor::SetPressure(const double pressure)
{
  impl_->SetPressure(pressure);
}
double ConstPressureReactor::GetPressure() const
{
  return impl_->GetPressure();
}

double
  ConstPressureReactor::GetMixtureSpecificHeat_Cp(const double state[])
{
...
double
  ConstPressureReactor::GetMixtureSpecificHeat_Cp(const double temperature,
                                             const double mass_fractions[],
                                             double *species_cp)
{
...
void ConstPressureReactor::GetSpeciesHydrogenCount(int num_atoms[]) const
{
  impl_->GetSpeciesHydrogenCount(num_atoms);
}
void ConstPressureReactor::GetSpeciesNitrogenCount(int num_atoms[]) const
{
  impl_->GetSpeciesNitrogenCount(num_atoms);
}
void ConstPressureReactor::GetSpeciesCarbonCount(int num_atoms[]) const
{
  impl_->GetSpeciesCarbonCount(num_atoms);
}
void ConstPressureReactor::GetSpeciesOxygenCount(int num_atoms[]) const
{
  impl_->GetSpeciesOxygenCount(num_atoms);
}
void ConstPressureReactor::GetSpeciesMolecularWeight(double molecular_weight[]) const
{
  impl_->GetSpeciesMolecularWeight(molecular_weight);
}

double ConstPressureReactor::GetGasConstant() const
{
