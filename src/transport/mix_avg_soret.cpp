                       ...
                       const double conductivity_multiplier)
{
  ...
  if(log_fptr == NULL) {
    printf("# ERROR: In MixAvgSoret::Initialize(),\n"
  ...
  if(input_files.size() != 3) {
    fprintf(log_fptr,
            "# ERROR: In MixAvgSoret::Initialize(),\n"
...
int MixAvgSoret::GetMixtureViscosity(const MassTransportInput &input,
...
int MixAvgSoret::GetSpeciesViscosity(const MassTransportInput &input,
                                       double *viscosity) const
{
...
int MixAvgSoret::GetMixtureConductivity(const MassTransportInput &input,
...
int MixAvgSoret::GetSpeciesConductivity(const MassTransportInput &input,
                                          double *conductivity) const
{
  if(input.temperature_ > 0.0) {
					   ...
					   double *species_lewis_numbers) const
{
  ...
  if(flag == NO_ERROR) {
	...
	// Compute species flux -- thermal
	species_mass_flux[flux_id] -= DTherm[k]*input.grad_temperature_[gradT_id]
          /input.temperature_;
...
double MixAvgSoret::omega_D(double t) const
{
...
double MixAvgSoret::omega_C(double t) const
{
  ...
  double MixAvgSoret::omega_mu (double t) const
  {
