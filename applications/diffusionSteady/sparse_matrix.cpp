...
SparseMatrix::SparseMatrix(const int num_rows, const int max_nonzeros)
{
  ...
  matrix_A_           = (double *)malloc(sizeof(double)*max_nonzeros);
...
SparseMatrix::~SparseMatrix()
{
  ...
  if(!is_first_product_) {
    Destroy_SuperMatrix_Store(&A_slu_); // SetupMatrixOnly
  }
}
void SparseMatrix::SetupFirstFactor()
{
...
void SparseMatrix::SetupMatrixOnly()
{
  dCreate_CompCol_Matrix(&A_slu_,
	                 num_rows_,
			 num_rows_,
			 max_nonzeros_,
			 matrix_A_,
...
int SparseMatrix::MatrixVectorProduct(const double matrix[],
                                      const double rhs[],
                                      double solution[])
{
  const int num_nonzeros = num_nonzeros_;
