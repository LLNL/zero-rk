...
void ComputeSootJacobian(void *user_data,
                         const double state[],
                         double soot_jacobian[])
{
  FlameParams *params = (FlameParams *)user_data;
  ...
  std::vector<double> prod_rate_orig;
  prod_rate_orig.assign(num_states, 0.0);
  std::vector<double> state_perturb, prod_rate_perturb;
  state_perturb.assign(num_states, 0.0);
  prod_rate_perturb.assign(num_states, 0.0);

  // Initialize perturbed state with original state
  for(int j=0; j<num_states; ++j)
    state_perturb[j] = state[j];

  // Compute baseline prod rate
  UpdateProductionRates(params,
                        &state[0],
                        &prod_rate_orig[0]);

  for(int j=0; j<nArom; ++j) {
    //Perturb species
    delta = alpha*state[AromInd[j]] + beta;
    state_perturb[AromInd[j]] = state[AromInd[j]] + delta;

    // Compute perturbed prod rate
    UpdateProductionRates(params,
                          &state_perturb[0],
                          &prod_rate_perturb[0]);
    ...
    for(int i=0; i<nArom; ++i) {
      soot_jacobian[AromInd[j]*num_states + AromInd[i]] =
        (prod_rate_perturb[AromInd[i]] - prod_rate_orig[AromInd[i]])/
        (state_perturb[AromInd[j]] - state[AromInd[j]]);
    }

    // Restore state and prod rate
    state_perturb[AromInd[j]] = state[AromInd[j]];

    for(int i=0; i<num_states; ++i)
      prod_rate_perturb[i] = 0.0;;
