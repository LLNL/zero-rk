...
int backSolve(double solution[], void *user_data)
{
  ...
  startTime = getHighResolutionTime();
  // copy the data to the sparse matrix RHS
  memcpy(cvp->sparseMtx->vecRHS,solution,cpysize);
  ...
  //copy solution to output
  memcpy(solution,cvp->sparseMtx->vecSoln,cpysize);
...
int sparse_oldjac_v(const double v[], double Jv[], void *user_data)
{
  int i,j,ix;
  cv_param *cvp=(cv_param *)user_data;
  int nsize = cvp->sparseMtxOld->nSize;
  double *sMptr=cvp->sparseMtxOld->mtxData;
  ...
  for (j=0; j<nsize; ++j)
  {
    if (v[j] != 0.)
    {
      for(i=cvp->sparseMtxOld->mtxColSum[j]; i<cvp->sparseMtxOld->mtxColSum[j+1]; ++i )
      {
        ix = cvp->sparseMtxOld->mtxRowIdx[i];
        Jv[ix] += v[j] * sMptr[i];
      }
    }
  }
