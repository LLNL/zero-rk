			    ...
			    void *user_data)
{
  ...
  // -------------------------------------------------------------------------
  // Compute the wall heat transfer contribution, if any
  for(int j=0; j<num_local_points; ++j) {
    int jglobal = j + my_pe*num_local_points;
    relative_volume_j = params->rel_vol_[j];
    // wall_term = 4*Nu/d^2
    // grid point thermal conductivity is the average of the conductivity at
    // the upstream and downstream interfaces
    rhs[(j+1)*num_states-1] -= 0.5*wall_term*
  ...
  for(int j=0; j<num_local_points; ++j) {
    ...
    for(int k=0; k<num_fuel_species; ++k) {
      local_sum -= rhs[j*num_states + params->fuel_species_id_[k]]*
  ...
  local_max = 0.0;
  for(int j=0; j<num_local_points; ++j) {
    velocity = fabs(params->rel_vol_[j]*y_ptr[(j+1)*num_states-2]);
  ...
  for(int j=0; j<num_local_points; ++j) {
    thermal_diffusivity =
      fabs(0.5*(params->thermal_conductivity_[j]+
           ...
           params->rel_vol_[j]/params->mixture_specific_heat_[j]);
			...
			void *user_data)
{
  ...
  std::vector<double> sens_rhs;
  sens_rhs.assign(num_local_states*num_reactions, 0.0);

  std::vector<double> rhs_ext_orig;
  rhs_ext_orig.assign(num_local_states,0.0);

  std::vector<double> y_orig;
  y_orig.assign(num_local_states, 0.0);

  // Get the original rhs
  ConstPressureFlame(y, ydot, user_data);
  for (int j=0; j<num_local_states; ++j)
    rhs_ext_orig[j] = ydot_ptr[j];
  ...
  for(int k=0; k<num_reactions; k++) {
    printf("Reaction %d of %d \n",k,num_reactions-1);
    // Perturb A factor
    int fwdId = params->mechanism_->getStepIdxOfRxn(k,1);
    int revId = params->mechanism_->getStepIdxOfRxn(k,-1);

    params->reactor_->SetAMultiplierOfStepId(fwdId, multiplier);
    if(revId >= 0 && revId < num_steps)
      params->reactor_->SetAMultiplierOfStepId(revId, multiplier);
    ...
    for(int j=0; j<num_local_states; j++) {
      sens_rhs[k*num_local_states+j] =  -(ydot_ptr[j] - rhs_ext_orig[j])/(multiplier - 1.0);
    }

    // Set A and solution back to original
    params->reactor_->SetAMultiplierOfStepId(fwdId, 1.0);
    if(revId >= 0 && revId < num_steps)
      params->reactor_->SetAMultiplierOfStepId(revId, 1.0);

    for (int j=0; j<num_local_states; ++j)
      y_ptr[j] = y_orig[j];

    // Solve for the sensitivity coefficients
    error_flag = AFSolve(&sens_rhs[k*num_local_states], user_data);

  } // loop over reactions

  // TO DO: Add normalization?

  // Get max sensitivity coefficients over the grid
  // Temperature only for now
  rxnSens_t *rxnSensList;
  rxnSensList = new rxnSens_t[num_reactions];
  double high, local_high;

  for(int k=0; k<num_reactions; k++) {

    local_high = 0.0;
    for(int j=0; j<num_local_points; ++j) {
      if(fabs(sens_rhs[k*num_local_states + j*num_states+num_species+1]) > local_high) {
