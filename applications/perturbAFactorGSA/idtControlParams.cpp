...
idtControlParams::idtControlParams(char *inpFile, int printLevel)
{
  ...
  checkFile     = parser.checkFile();
  gsaMatrixFile = parser.gsaMatrixFile();
...
void idtControlParams::setROPMultiplierOfRxn(const int rxnId,
                                             const double afactor_mult)
{
  int fwdId = mech->getStepIdxOfRxn(rxnId,1);
  int revId = mech->getStepIdxOfRxn(rxnId,-1);

  ropMultiplier[fwdId] *= afactor_mult;
  if(revId >= 0 && revId < nStep) {
    ropMultiplier[revId] *= afactor_mult;
  }
}

void idtControlParams::setROPMultipliers(const double afactor_mult[])
{
  const int num_reactions = nRxn;
  int fwdId,revId;

  for(int j=0; j<num_reactions; ++j) {
    fwdId = mech->getStepIdxOfRxn(j,1);
    revId = mech->getStepIdxOfRxn(j,-1);

    ropMultiplier[fwdId] = afactor_mult[j];
    if(revId >= 0 && revId < nStep) {
      ropMultiplier[revId] = afactor_mult[j];
    }
  }
}
...
void idtControlParams::setupCVode(GSA_AFactorIFP &parser)
{
  int flag;
...
void idtControlParams::setupCVodeUserParams(GSA_AFactorIFP &parser)
{
  int j;
  ...
  odeUserParams.sparseMtx =
    (Jsparse *)alloc_Jsparse(*mech,
                             zerork_ctrl.precThresh,
                             zerork_ctrl.doILU,
                             zerork_ctrl.doFakeUpdate,
                             zerork_ctrl.precThreshType,
                             zerork_ctrl.partialPivotThresh,
                             zerork_ctrl.permutationType);
...
void idtControlParams::setupIdtState(GSA_AFactorIFP &parser)
{
  int j,spcIdx;
  ...
  if(fuelOxygenBal > 0.0 && initPhi > 0.0) {
    ...
    printf("       moles of atomic-O per mole of fuel is %.18g\n",
           fuelOxygenBal);
    printf("       Contact the ZeroRK developers to add this feature.\n");
    exit(-1);
  }
  ...
  if(initPhi <= 0.0 && initEgr > 0.0) {
    ...
    printf("       egr mass fraction = %.18g\n",
           initEgr);
    printf("       Contact the ZeroRK developers to add this capability.\n");
    exit(-1);
  }
...
void idtControlParams::setupZerorkParams(GSA_AFactorIFP &parser)
{
  zerork_ctrl.doFakeUpdate       = parser.doFakeUpdate();
  zerork_ctrl.doILU              = parser.doILU();
  zerork_ctrl.strictSamePattern  = parser.strictSamePattern();

  zerork_ctrl.precThreshType     = parser.precThreshType();
  zerork_ctrl.permutationType    = parser.permutationType();

  zerork_ctrl.precThresh         = parser.precThresh();
  zerork_ctrl.partialPivotThresh = parser.partialPivotThresh();
  zerork_ctrl.permThresh         = parser.permThresh();
}
