                               ...
                               const int num_lines_skipped)
{
  ...
  if(is_csv) {
    return_flag = ReadCSVFile(filename,
                              ignore_chars,
                              num_lines_skipped);
  } else {
    return_flag = ReadFile(filename,
                           ignore_chars);
  }
  // TO DO: add handling for a single point to be treated as a constant
  if(return_flag < 2) {
...
int VolumeFromFile::ReadCSVFile(const char filename[], 
                                const char *ignore_chars,
                                const int num_lines_skipped)
{
  size_t ignore_pos;
      ...
      if(sub_line.size() > 2) { // minimum length for two values
        ...
        if(num_values_read != 2) {
          ...
          printf("      %s\n",line.c_str());
           ...
           if(volume_read <= 0.0) {
	     printf("# ERROR: non-positive volume found in file %s\n",
	            filename);
             printf("#        at line %d: t = %.18g, vol = %.18g",
		    num_lines_read, time_read, volume_read);
             fflush(stdout);
             exit(-1);
	   }
...
void VolumeFromFile::SetFileExtrema()
{
  max_volume_ = -1.0e300;
  min_volume_ = 1.0e300;
  time_at_min_volume_ = -1.0e300;
  time_at_max_volume_ = -1.0e300;

  for(size_t j=0; j<t_.size(); ++j) {
    if(volume_[j] < min_volume_) {
      min_volume_         = volume_[j];
      time_at_min_volume_ = t_[j];
    }
    if(volume_[j] > max_volume_) {
      max_volume_         = volume_[j];
      time_at_max_volume_ = t_[j];
    }
  }
  if(min_volume_ <= 0.0) {
    printf("# ERROR: Minimum volume in file is less than or equal to zero.\n");
    printf("#        min vol = %.18g\n",min_volume_);
    fflush(stdout);
    exit(-1);
  }
