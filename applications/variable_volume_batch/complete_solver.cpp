                     ...
                     FILE* err_fptr)
{
  ...
  //if (CheckFlag((void *)cvode_memory, "CVodeCreate", 0)) exit(-1);
  ...
  error_flag = CVodeSetErrFile(cvode_memory,err_fptr);
  if (CheckFlag(&error_flag, "CVodeSetErrFile", 1)) exit(-1);
...
// results[0] = time at minimum volume in file
// results[1] = min volume in file
// results[2] = recorded time for min volume
// results[3] = recorded min volume
// results[4] = pressure at recorded min volume
// results[5] = temperature at recorded min volume
// results[6] = time at max dp/dt
// results[7] = max dp/dt
// results[8] = final simulation time
                        ...
                        double *simulation_time)
{
  ...
  double current_time;
  double final_time, cvode_stop_time;
  int num_steps=0;
  int error_flag;
  ...
  // Two options, define minimum value
  double min_volume = user_data->GetMinVolume();
  double time_at_min_volume = user_data->GetTimeAtMinVolume();

  bool reached_min_volume=false;
  ...
  *simulation_time = 0.0;
  ...
  if(final_time < time_at_min_volume) {
    printf("# WARNING: In SolveVariableVolume(...),\n");
    printf("#          final time is less than time at min volume\n");
    printf("#              final time      = %.18g [s]\n",final_time);
    printf("#              time at min vol = %.18g [s]\n",time_at_min_volume);
    final_time = 100.0*(time_at_min_volume-current_time);
    printf("#          final time reset to %.18g [s]\n",final_time);
  }
  //
  cvode_stop_time = time_at_min_volume;

  while(current_time < final_time) {
    previous_time = current_time;
    previous_pressure = current_pressure;

    int cv_mode = CV_NORMAL;
    if(reached_min_volume || thist_params.record_prehistory) {
      cv_mode = CV_ONE_STEP;
    }
    ...
    if(CheckFlag(&error_flag, "CVODE ERROR", 1) ||
      ...
      std::string prob_file_name = user_data->GetParser()->outputFile();
      char buffer[48];
      snprintf(buffer,48,"_%08d_failed_state",user_data->GetTaskNum());
      prob_file_name += std::string(buffer);
      FILE *prob_fptr = fopen(prob_file_name.c_str(),"w");
      PrintStateDerivatives(prob_fptr,
                            current_time,
                            cvode_state,
                            user_data);
      fclose(prob_fptr);
    ...
    if(current_time >= time_at_min_volume && reached_min_volume == false) {
      results[0] = time_at_min_volume; // from volume time history file
      results[1] = min_volume;         // from volume time history file
      // actual recorded values for minimum volume to allow for the alternative
      // of using the numerically interpolated minimum volume instead of the
      // tabulated points
      results[2] = current_time;
      results[3] = user_data->GetVolume(current_time,NV_DATA_S(cvode_state));
      results[4] = current_pressure;
      results[5] = user_data->GetTemperature(NV_DATA_S(cvode_state));

      cvode_stop_time = final_time;
      reached_min_volume = true;
    }
  }
  results[6] = time_max_dp_dt;
  results[7] = max_dp_dt;
  results[8] = current_time;
  ...
  if(thist_params.echo_stdout) {
    ...
    printf("# Time at min volume (table) [s]: %14.7e\n", results[3]);
                           ...
                           UserData *params)
{
  ...
  fprintf(out,"# state[id]             name: <state_value>  <state_derivative>\n");
  for(int j=0; j<num_states; ++j) {
    fprintf(out,"# state[%2d] %16s: %.18g  %.18g\n",
           j,
