...
#include "interfaces/radau5_cpp/radau_cpp.h"
#include "nvector/nvector_serial.h"
#include "utility_funcs.h"

static int RadauSolverMonitorFn(int nsteps, double x, double h, N_Vector y,
                                N_Vector ydot, void* cb_fn_data) {
  RadauSolver* solver = static_cast<RadauSolver*>(cb_fn_data);
  return solver->MonitorFn(nsteps, x, h, y, ydot);
}

RadauSolver::RadauSolver(ReactorBase& reactor)
    : SolverBase(reactor),
      reactor_ref_(reactor),
      cb_fn_(nullptr),
      cb_fn_data_(nullptr) {}

int RadauSolver::Integrate(const double end_time) {
  ...
  radau_cpp::radau s(num_variables * num_batches, state);
  ...
  s.set_jac_complex_decomp_fcn(ReactorComplexJacobianFactor);
  s.set_jac_complex_solve_fcn(ReactorComplexJacobianSolve);
  s.set_output_fcn(RadauSolverMonitorFn, this);
...
void RadauSolver::AdjustWeights() {
  int num_batches = reactor_ref_.GetNumBatchReactors();
...
void RadauSolver::SetCallbackFunction(zerork_callback_fn fn, void* cb_fn_data) {
  cb_fn_ = fn;
  cb_fn_data_ = cb_fn_data;
}

int RadauSolver::MonitorFn(int nsteps, double x, double h, N_Vector y,
                           N_Vector ydot) {
  int cb_flag = 0;
