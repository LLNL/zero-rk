  ...
  for(j=mpi_rank; j<idt_ctrl.getRunTotal(); j+=mpi_size, local_idx++)
    {
      ...
      systemParam.colPermTime   = 0; systemParam.jacFactorTime = 0;
...
//      fflush(thistFilePtr);
  ...
  for(j=0; j<idt_ctrl.getNumThreshRuns(); j++)
    {
        global_avgTime[j]/=nRunsPerThresh;
      fprintf(idtFilePtr,"%14.7e  %14.7e  %14.7e  %14.7e\n",
                idt_ctrl.getThreshFromId(j),global_avgTime[j],global_minTime[j],global_maxTime[j]);
    }
  fflush(idtFilePtr);
  fclose(idtFilePtr);
  fclose(thistFilePtr);
