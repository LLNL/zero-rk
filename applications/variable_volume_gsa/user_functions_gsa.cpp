...
UserData::UserData(const char filename[], const bool write_parser_log)
{
  int error_flag;

  parser_ = new VariableVolumeGSAIFP(filename);
  if(parser_ == NULL) {
  ...
  volume_    = new VolumeFromFile(parser_->volumeFile().c_str(),
                                  interpType,
  ...
  initial_time_ = parser_->initTime();
  ...
  num_new_jacobians = 0;
  // heat release rate quadrature variables.  The first is always just
  // the heat release rate to compute the total heat released.
  num_quadratures_ = 1 + parser_->hrrIntegralPower().size();
  heat_release_rate_powers_.assign(num_quadratures_, 1.0);
  for(int j=1; j<num_quadratures_; ++j) {
    heat_release_rate_powers_[j] = parser_->hrrIntegralPower()[j-1];
  }
  quadrature_rtol_ = parser_->quadrature_rtol();
  quadrature_atol_ = parser_->quadrature_atol();
  quadrature_controls_step_ = parser_->quadrature_controls_step();
  quadrature_ = N_VNew_Serial(num_quadratures_);

  for(int j=0; j<num_quadratures_; ++j) {
    NV_Ith_S(quadrature_,j) = 0.0;
  }
...
int UserData::BuildInitialState()
{
  ...
  initial_moles = (parser_->initPres()*initial_volume)/
    (USER_GAS_CONSTANT*parser_->initTemp());

  // parse the composition information
  mole_frac_sum = 0.0;
  for(map_iter = parser_->initComp().begin();
      map_iter != parser_->initComp().end();
      ++map_iter) {
    ...
    if(species_id < 0) {
      printf("ERROR: could not find state %s for initComp species %s\n",
             search_name.c_str(),map_iter->first.c_str());
      fflush(stdout);
      return -1;
    }
    tracked_species_ids_.push_back(species_id);
    initial_state_[species_id] = map_iter->second;
    mole_frac_sum += map_iter->second;
  }
  ...
  if(parser_->compType() == std::string("MASS_FRACTION")) {
    ...
    printf("         assuming initComp is specified as mole fractions.\n");
    fflush(stdout);
  }
  ...
  initial_state_[num_species+1] = parser_->initTemp()/parser_->refTemp();
...
// qdot[0] is assumed to be the heat release rate
int ChemicalHRRIntegrand(realtype t,
                         N_Vector y,
                         N_Vector qdot,    // integrand
                         void *params)
{
  UserData *user_data = (UserData *)params;
  const int num_integrands = user_data->num_quadratures_;
  double *state      = NV_DATA_S(y);    // pointers to data array for N_Vector
  double *integrand  = NV_DATA_S(qdot);

  double heat_release_rate;
  user_data->GetReactor()->GetChemicalHeatReleaseRate(t,
                                                      state,
                                                      &heat_release_rate);
  for(int j=0; j<num_integrands; ++j) {
    integrand[j] = pow(heat_release_rate,
                       user_data->heat_release_rate_powers_[j]);
  }
...
double ChemicalHeatReleaseRatePow2(const double t,
                                   const double y[],
                                   void *params)
{
  double heat_release_rate = ChemicalHeatReleaseRate(t, y, params);
  return pow(heat_release_rate, 2);
}
double ChemicalHeatReleaseRatePow4(const double t,
                                   const double y[],
                                   void *params)
{
  double heat_release_rate = ChemicalHeatReleaseRate(t, y, params);
  return pow(heat_release_rate, 4);
}
double ChemicalHeatReleaseRatePow6(const double t,
                                   const double y[],
                                   void *params)
{
  double heat_release_rate = ChemicalHeatReleaseRate(t, y, params);
  return pow(heat_release_rate, 6);
}
double ChemicalHeatReleaseRatePow8(const double t,
                                   const double y[],
                                   void *params)
{
  double heat_release_rate = ChemicalHeatReleaseRate(t, y, params);
  return pow(heat_release_rate, 8);
}
double ChemicalHeatReleaseRatePow10(const double t,
                                   const double y[],
                                   void *params)
{
  double heat_release_rate = ChemicalHeatReleaseRate(t, y, params);
  return pow(heat_release_rate, 10);
}
